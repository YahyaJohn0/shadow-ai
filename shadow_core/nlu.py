...
Combines rule-based patterns with AI-powered classification for accurate intent recognition
"""
...
class NLU:
    """
    Advanced NLU system with rule-based patterns and AI fallback
    """
    ...
    def _initialize_patterns(self) -> Dict[str, List[Dict]]:
         """Initialize rule-based patterns for common intents"""
    ...
    async def classify(self, text: str) -> Tuple[str, Dict[str, Any]]:
        ...
        if rule_result.confidence > 0.8:
            logger.info(f"NLU: Rule-based match - {rule_result.intent} (confidence: {rule_result.confidence})")
...
class SimpleNLU:
    ...
    async def classify(self, text: str) -> Tuple[str, Dict[str, Any]]:
        """Classify intent using only rule-based methods"""
        text_lower = text.lower().strip()
